...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/JerryXu-code/3x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        LOGI "更新完成，面板已自动重启"
        exit 0
    fi
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/JerryXu-code/3x-ui/master/install.sh"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/JerryXu-code/3x-ui/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/JerryXu-code/3x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
